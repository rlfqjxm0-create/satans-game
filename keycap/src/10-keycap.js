...
const S={items:[], shape:"cherry", mat:"resin", color:"#FFB8D0", charPos:"inside", deco:"cat", decoMat:"gloss", decoColor:"#FFFFFF", glitter:"star", base:"clear", baseColor:"#CFE3FF", sw:"mango", rgb:"rainbow", rgbColor:"#FF6FB5", bg:"peach", bgImg:null, name:"", palette:[], sound:true};
const OPT={
...
function glitterGeo(kind){
  ...
  const g=new THREE.ExtrudeGeometry(s,{depth:0.14,bevelEnabled:true,bevelThickness:0.07,bevelSize:0.07,bevelSegments:2,curveSegments:10}); g.center(); GLIT_GEO[kind]=keep(g); return g;
}
function glitterMat(kind){
  if(GLIT_MAT[kind]) return GLIT_MAT[kind];
  let m;
  if(kind==="star") m=new THREE.MeshStandardMaterial({color:lin("#FFD66B"),metalness:0.75,roughness:0.18,emissive:lin("#FFB938"),emissiveIntensity:0.85});
  ...
  GLIT_MAT[kind]=keep(m); return m;
}
/* The glitter is fenced by the cap's own cross-section, not by a square box - a square let flakes
   wander outside the heart (and the round cap's corners). FENCE[a] = distance from the centre to the
   outline along angle a, for a unit-size section; the heart's notch is concave, so it also looks a few
   degrees either side. Checked for all four shapes: a flake centre on the fence stays >=1.3 inside the wall. */
...
function fenceTable(p){const key=p.sec+p.n0+"/"+p.n1; if(FENCES[key]) return FENCES[key];
  ...
  for(let k=0;k<FENCE_N;k++){const a=k/FENCE_N*Math.PI*2, dx=Math.cos(a), dz=Math.sin(a); let best=Infinity;
    ...
    R[k]=best}
  const win=p.sec==="heart"?6:0, out=new Float32Array(FENCE_N);
  for(let k=0;k<FENCE_N;k++){let m=R[k]; for(let j=-win;j<=win;j++) m=Math.min(m,R[(k+j+FENCE_N)%FENCE_N]); out[k]=m}
  return (FENCES[key]=out)}
...
const fenceLim=(T,a,w)=>{const r=fenceAt(T,a)*w; return Math.max(0,Math.min(r*0.72,r-1.3))};
/* a soft halo + thin cross flare on every star flake; additive, so it reads as light, and each one
   twinkles at its own pace. Shares the flakes' position array, so it costs one small draw call. */
let GLOW_TEX=null, GLOW_MAT=null;
function starGlow(pos,N){
  if(!GLOW_TEX) GLOW_TEX=keep(canvasTex(64,64,(x)=>{const g=x.createRadialGradient(32,32,0,32,32,32); g.addColorStop(0,"rgba(255,255,255,1)"); g.addColorStop(0.14,"rgba(255,244,200,.95)");
    g.addColorStop(0.4,"rgba(255,206,100,.32)"); g.addColorStop(1,"rgba(255,190,80,0)"); x.fillStyle=g; x.fillRect(0,0,64,64);
    x.globalCompositeOperation="lighter"; for(const [w,h] of [[64,3.2],[3.2,64]]){const l=x.createLinearGradient(32-w/2,32-h/2,32+w/2,32+h/2); l.addColorStop(0,"rgba(255,240,200,0)"); l.addColorStop(0.5,"rgba(255,252,236,1)"); l.addColorStop(1,"rgba(255,240,200,0)"); x.fillStyle=l; x.fillRect(32-w/2,32-h/2,w,h)}}));
  if(!GLOW_MAT) GLOW_MAT=keep(new THREE.PointsMaterial({map:GLOW_TEX,size:2.9,sizeAttenuation:true,transparent:true,depthWrite:false,blending:THREE.AdditiveBlending,vertexColors:true}));
  const g=new THREE.BufferGeometry(); g.setAttribute("position",new THREE.BufferAttribute(pos,3)); g.setAttribute("color",new THREE.BufferAttribute(new Float32Array(N*3),3));
  const pts=new THREE.Points(g,GLOW_MAT); pts.renderOrder=4; pts.frustumCulled=false; return pts;
}
function twinkle(pts,N,t){const col=pts.geometry.attributes.color.array;
  for(let i=0;i<N;i++){const s=0.5+0.5*Math.sin(t*(1.6+hash(i+300)*2.2)+hash(i+500)*6.28), k=0.3+0.7*s*s; col[i*3]=k; col[i*3+1]=k*0.92; col[i*3+2]=k*0.72}
  pts.geometry.attributes.color.needsUpdate=true; pts.geometry.attributes.position.needsUpdate=true}
function makeParticles(p,g){
  if(S.glitter==="none"||!(S.mat==="resin"||S.mat==="jelly")) return null;
  const kind=S.glitter, N={star:60,snow:30,heart:28,aurora:46}[kind], scale={star:0.42,snow:1.1,heart:1.0,aurora:0.8}[kind];
  ...
  const fence=fenceTable(p);
  for(let i=0;i<N;i++){const v=r()*0.3, a=r()*Math.PI*2, rr=Math.sqrt(r())*fenceLim(fence,a,lerp(p.bw,p.tw,Math.pow(v,1.2))/2);
    pos[i*3]=Math.cos(a)*rr; pos[i*3+1]=0.9+v*p.h*0.6; pos[i*3+2]=Math.sin(a)*rr;
    for(let a=0;a<3;a++){rot[i*3+a]=r()*6.28; spin[i*3+a]=(r()*2-1)*0.4}}
  const sys={mesh,pos,vel,rot,spin,N,p,scale,fence,dummy:new THREE.Object3D()};
  if(kind==="star") sys.glow=starGlow(pos,N);
  return sys;
}
...
function stepParticles(dt,t){
  if(!partSys) return; const {mesh,pos,vel,rot,spin,N,p,scale,fence,dummy}=partSys, grav=S.glitter==="snow"?2.6:4.2;
  for(let i=0;i<N;i++){const ix=i*3;
    ...
    const floor=0.9, y=pos[ix+1], v=clamp((y-floor)/(p.h*0.85));
    ...
    {const px=pos[ix], pz=pos[ix+2], rr=Math.hypot(px,pz);
      if(rr>1e-6){const lim=fenceLim(fence,Math.atan2(pz,px),lerp(p.bw,p.tw,Math.pow(v,1.2))/2);
  ...
  if(partSys.glow) twinkle(partSys.glow,N,t);
}
...
function loadPack(id){
  ...
  if(!dec) return (PACK_LOAD[id]=Promise.resolve());
  return (PACK_LOAD[id]=Promise.all(names.map(n=>fetch(`sounds/${id}/${n}.wav`).then(r=>r.ok?r.arrayBuffer():Promise.reject(r.status))
    ...
    .then(list=>{const pb={key:[],space:[],enter:[],shift:[],back:[]}; let got=0;
      ...
      if(got&&pb.key.length) PACK_BUF[id]=pb}));   // nothing loaded (opened as a file) -> the synth below stays
}
let lastKeys=[];
...
function sfx(kind){if(!S.sound) return; const a=ac(); if(!a) return; loadPack(S.sw);
  if(PACK_BUF[S.sw]){if(kind==="down") playPack(a); return}   // a recording already holds the whole keystroke
...
function readHash(){try{const m=location.hash.match(/#k=(.+)/); if(!m) return; const o=JSON.parse(decodeURIComponent(escape(atob(decodeURIComponent(m[1])))));
  ...
  Object.assign(S,{base:opt(OPT.base,o.bs,S.base),baseColor:col(o.bc,S.baseColor),shape:opt(PROFILES,o.s,S.shape),mat:opt(OPT.mat,o.m,S.mat),color:col(o.c,S.color),
    ...
    sw:opt(OPT.sw,o.w,S.sw),rgb:opt(OPT.rgb,o.r,S.rgb),rgbColor:col(o.rc,S.rgbColor),bg:opt(OPT.bg,o.b,S.bg),name:typeof o.n==="string"?o.n.slice(0,6):""}); $("name").value=S.name}catch(e){}}
...
readHash(); setBg(); renderUI(); rebuild(); (window.requestIdleCallback||setTimeout)(()=>loadPack(S.sw));
