...
if(DESK){(function(){
  const D=window.deskAPI||{move(){},zoom(){},hit(){},menu(){},on(){},drag(){}};
  ...
  addEventListener("pointermove",e=>{mouse=[e.clientX,e.clientY]; need=true; if(!down) return; stop(e);
    if(Math.hypot(e.screenX-down.x,e.screenY-down.y)>4&&!down.moved){down.moved=true; if(down.b===0&&D.drag) D.drag(true)}   // main follows the cursor from here
    if(!down.moved) return;
    const dx=e.screenX-down.lx, dy=e.screenY-down.ly; down.lx=e.screenX; down.ly=e.screenY; idle=0;
    if(down.b===2){rotY+=dx*0.012; rotX=clamp(rotX+dy*0.006,-0.3,0.45)}},true);
  addEventListener("pointerup",e=>{if(!down) return; stop(e); const d=down; down=null; idle=0; if(d.b===0&&d.moved&&D.drag) D.drag(false);
  ...
  DRAW_GATE=now=>{if(down&&down.b===0&&down.moved) return false;   // being dragged: the picture doesn't change, redrawing only flickers
    const busy=down||press>0.01||Math.abs(pressV)>0.02||idle<3;
