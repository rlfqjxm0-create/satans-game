# 사탄과 장난감 (satans-game) — 작업 지침

> 사이트 이름은 **「사탄과 장난감」**, 게임은 「사탄 세탁소」·「사탄 클리커」(2026-09-22에 바뀜 — 그전엔 사탄의 장난감·사탄의 세탁·사탄의 키캡). 레포 이름·폴더 이름(`laundry`, `keycap`)·프로그램 파일 이름(`SatanKeycap-Setup.exe`, 상태 폴더 `사탄의 키캡`)은 그대로다.
...
| `laundry/index.html` | 게임 ①「사탄 세탁소」. 전체 게임이 이 한 파일 | 2440 |
| `keycap/` | 게임 ②「사탄 클리커」(3D 키캡 만들기). **`src/`를 고치고 `build.py`로 `index.html`을 만든다** — 자세한 건 `keycap/CLAUDE.md` | — |
